...
| **R-B** | `Candidate.obligations_created` holds prose where the prompt and the contract both say ids | The canonical write boundary now REJECTS it (`REFERENCE_NOT_AN_ID`). The producer emitting prose is s02 physical-producer meaning, which S-4 owns. S-3's boundary is correct and does not wait for it | **S-4** | `test_s01_s02_window.test_R_B_the_recorded_s02_response_violates_the_reference_contract` — every recorded case is rejected and attributed | A compatibility shim, a free-string weakening, or prose reinterpreted as an unresolved id is required for the pipeline to progress | **OPEN — EXPECTED NEXT-STEP BLOCKER** |
